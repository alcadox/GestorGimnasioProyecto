...
{
    public partial class FormularioRenovar : Form
    {
        ...
        public FormularioRenovar(string nombrePersona, string apellidosPersona, string idPersona)
        {
            InitializeComponent();
        ...
        private void buttonConfirmar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (var conexion = new MySqlConnection(conexionString))
                {
                    ...
                    string sql = "UPDATE clients SET tipo_pago = @tipoPago, fecha_inicio = @fechaInicio, fecha_fin = @fechaFin WHERE id = @id";
                    using (var cmd = new MySqlCommand(sql, conexion))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@id", Convert.ToInt32(clienteId));
